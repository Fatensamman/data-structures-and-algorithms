...
const getNames = (arr) => {
  let newA = arr.map(item => item.name.split('').reverse().join(''));
  return newA;
};
...
const count = (target, input) => {
  let newA = input.map(arr => arr.filter(i => i === target));
  let n = newA.reduce((a, el) => a += el.length, 0);
  return n;
};
...
const totalSum = (input) => {
  let newA = input.map(el => el.reduce((acc, i) => acc += i, 0))
  return newA.reduce((acc, i) => acc += i)
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let newA = input.map(ele => ele.filter(el => typeof (el) === 'number' && !(el % 5)))
  let n = newA.map(el => el.map(el => Math.pow(2, el)))
  return n;
};
...
let findShortest = (data) => {
  let m = data.sort((a,b)=> a.height-b.height);
  return m[0].name;
  };
