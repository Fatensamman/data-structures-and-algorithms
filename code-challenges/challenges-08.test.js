...
function returnTen(str){
  let arr = str.split('').splice(str.length-10,str.length);
  return arr;
}
...
const containsAnd = (arr) => {
  let i = arr.filter(el=> el.includes('and')
  );
  return i;
};
...
const filterStringsWithVowels = (arr) => {
  let i = arr.filter(el=> /[aioeu]/g.test(el)
  )
  return i;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let i = arr.filter(elem=> !forbiddenValues.includes(elem));
  return i;
};
